...
use mofish_core::db::{self, Book, Stock};
...
pub fn run_tui() {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            // 主布局: header(3行) + content(剩) + status(3行)
            let main_chunks = Layout::default()
                .direction(Direction::Vertical)
                .constraints([
                    Constraint::Length(3),  // header (tab)
                    Constraint::Min(0),     // content
                    Constraint::Length(3),  // status / command bar
                ])
                .split(size);
